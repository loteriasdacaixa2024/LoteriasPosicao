...
from services.modelos_service import ModelosLotomaniaService

modelos_bp = Blueprint('modelos', __name__)

@modelos_bp.route('/')
def modelos_index():
    modelos = ModelosLotomaniaService.listar_modelos()
...
def api_gerar_modelo(modelo_id: int):
    """Gera 14 apostas (50 dezenas cada) para o modelo especificado (1-6)."""
    if modelo_id not in range(1, 7):
        return jsonify({"error": "Modelo inválido. Use 1-6."}), 400
    try:
        resultado = ModelosLotomaniaService.gerar_apostas_modelo(modelo_id)
...
def api_backtesting():
    ...
    try:
        resultado = ModelosLotomaniaService.backtesting_modelos()
