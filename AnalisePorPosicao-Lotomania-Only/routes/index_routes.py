from flask import Blueprint, render_template, jsonify, request
from services.api_lotomania_service import ApiLotomaniaService

index_bp = Blueprint('index', __name__)

@index_bp.route('/')
def index():
    return render_template('index.html')

@index_bp.route('/api/status-banco', methods=['GET'])
def api_status_banco():
    try:
        return jsonify({"status": "success", **ApiLotomaniaService.status_banco()})
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500

@index_bp.route('/api/sincronizar', methods=['POST'])
def api_sincronizar():
    try:
        data = request.get_json(silent=True) or {}
        modo = data.get("modo", "completo")
        limite = int(data.get("limite", 60))
        teto = data.get("teto_concurso")
        teto = int(teto) if teto else None
        resultado = ApiLotomaniaService.sincronizar_banco(
            modo=modo, limite=limite, teto_concurso=teto,
        )
        return jsonify(resultado)
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500


import os as _os
import sys as _sys
from models.shared import db
from models.sorteio_lotomania import SorteioLotomania
_SHARED = _os.path.abspath(_os.path.join(_os.path.dirname(__file__), "..", "..", "_shared"))
if _SHARED not in _sys.path:
    _sys.path.insert(0, _SHARED)
from caixa_excel.routes_factory import register_premiacao_caixa
register_premiacao_caixa(index_bp, modality_key="lotomania", sorteio_model=SorteioLotomania, db=db)
