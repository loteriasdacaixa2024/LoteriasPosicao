#!/usr/bin/env python
# -*- coding: utf-8 -*-
r"""Sincroniza histórico completo Lotomania (1..último API). Porta 5154."""
...
def salvar(concurso, dados):
    ...
    db.session.merge(SorteioLotomania(
        concurso=concurso,
...
def status(ultimo):
    ...
    mn = db.session.query(db.func.min(SorteioLotomania.concurso)).scalar() or 0
    mx = db.session.query(db.func.max(SorteioLotomania.concurso)).scalar() or 0
    grav = db.session.query(SorteioLotomania.concurso).filter(
        SorteioLotomania.concurso >= 1, SorteioLotomania.concurso <= ultimo
...
def main():
    ...
    with app.app_context():
        ...
        if not ultimo:
            ...
            return 1
        print("=" * 60)
        print("SINCRONIZAÇÃO LOTOMANIA — histórico completo")
        print("Banco: instance/lotomania.db | Porta: 5154")
        ...
        while True:
            ...
            for c in faltantes[:limite]:
                print(f"[API] Lotomania {c}...", end="", flush=True)
                try:
                    d = buscar(c, timeout)
                    if d and salvar(c, d):
                        db.session.commit()
