...
class CicloDiaDeSorteService:
    @staticmethod
    def obter_ciclo_atual():
        """
        Calcula os ciclos do Dia de Sorte em ordem cronológica.
        O ciclo reinicia toda vez que todas as 31 dezenas distintas forem sorteadas.
        Retorna as dezenas que já saíram no ciclo atual e as que faltam.
        """
        try:
            sorteios = db.session.query(SorteioDiaDeSorte).order_by(
                SorteioDiaDeSorte.concurso.asc()
            ).all()
        ...
        for s in sorteios:
            dezenas_sorteio = s.dezenas()  # Retorna o set {d1, d2, d3, d4, d5, d6, d7}
            dezenas_sorteadas.update(dezenas_sorteio)
            concursos_no_ciclo += 1

            if len(dezenas_sorteadas) == 31:
                # O ciclo completou!
                ciclo_num += 1
                dezenas_sorteadas = set()
                concursos_no_ciclo = 0
        ...
        dezenas_faltantes = sorted(list(set(range(1, 32)) - dezenas_sorteadas))
