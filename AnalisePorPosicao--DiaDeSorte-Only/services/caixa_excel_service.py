...
def importar_arquivo(path: Path | None = None) -> Dict[str, Any]:
    ...
    stamp = _agora()
    ja = {int(r[0]) for r in db.session.query(CaixaExcelPremiacaoDiaDeSorte.concurso).all()}
    inseridos = 0
    pulados = 0
    for rec in parsed:
        conc = int(rec["concurso"])
    ...
    return {
        ...
        "inseridos": inseridos,
        "pulados_ja_no_banco": pulados,
        "atualizado_em": stamp,
        "message": (
            f"Excel complementar: {inseridos} inserido(s), "
            f"{pulados} já no banco (prevalecem). Dezenas não alteradas."
        ),
    }
