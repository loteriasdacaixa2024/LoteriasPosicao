...
def create_app():
    app = Flask(__name__)
    app.config['SECRET_KEY'] = 'sniper_maismilionaria_secret'
    basedir = os.path.abspath(os.path.dirname(__file__))
    app.config['SQLALCHEMY_DATABASE_URI'] = (
        'sqlite:///' + os.path.join(basedir, 'instance', 'maismilionaria.db')
    )
    app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
    app.config['TEMPLATES_AUTO_RELOAD'] = True
    db.init_app(app)
    extend_config_app(app)
    cc_extend_app(app, 'maismilionaria')
    extend_nav_app(app, 'maismilionaria')
    ...
    extend_posicao_app(app, 'maismilionaria')
    extend_resumo_modalidade_app(app)
    extend_ciclo_cobertura_app(app, 'maismilionaria')
    app.register_blueprint(index_bp)
    app.register_blueprint(analise_bp,     url_prefix='/analise')
    register_conferencia_extras(conferencia_bp, 'maismilionaria')
    ...
    with app.app_context():
        import models.sorteio_maismilionaria
    ...
    start_auto_sync_once(
        app,
        "services.api_maismilionaria_service",
        "ApiMaisMilionariaService",
    )
...
if __name__ == '__main__':
    app = create_app()
    porta = int(os.getenv('PORT', 5157))
    print("===========================================================")
    print(f"[OK] Servidor +Milionária rodando na porta {porta}")
