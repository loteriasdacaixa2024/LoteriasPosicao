...
BASE_URL = "https://servicebus2.caixa.gov.br/portaldeloterias/api/maismilionaria/"


class ApiMaisMilionariaService:
    @staticmethod
    def get_headers():
    ...
    def _get(url: str, timeout: int = 30):
        kwargs = {"headers": ApiMaisMilionariaService.get_headers(), "timeout": timeout}
        try:
    ...
    def buscar_ultimo_concurso(timeout: int = 30) -> int:
        try:
            r = ApiMaisMilionariaService._get(BASE_URL, timeout=timeout)
    ...
    def buscar_concurso_especifico(numero: int) -> Optional[dict]:
        try:
            r = ApiMaisMilionariaService._get(f"{BASE_URL}{numero}")
            return r.json() if r.status_code == 200 else None
        except Exception:
            return None

    @staticmethod
    def _extrair_trevos(dados: dict) -> Optional[List[int]]:
    ...
    def status_banco() -> Dict[str, Any]:
        ...
        min_local = db.session.query(db.func.min(SorteioMaisMilionaria.concurso)).scalar()
        max_local = db.session.query(db.func.max(SorteioMaisMilionaria.concurso)).scalar()
        ultimo_api = ApiMaisMilionariaService.buscar_ultimo_concurso(timeout=8)
        alvo = ultimo_api or max_local or 0
        faltantes = 0
        if alvo:
            gravados = db.session.query(SorteioMaisMilionaria.concurso).filter(
                SorteioMaisMilionaria.concurso >= 1, SorteioMaisMilionaria.concurso <= alvo
            ).count()
    ...
    def sincronizar_banco(cls, modo: str = "completo", limite: int = 60, teto_concurso: Optional[int] = None) -> Dict[str, Any]:
        ...
        for concurso in lote:
            print(f"[API Caixa] +Milionária {concurso}/{ultimo_oficial}...", end="", flush=True)
