...
def api_status_banco():
    try:
        return jsonify({"status": "success", **ApiMegaSenaService.status_banco()})
...
def api_sincronizar():
    try:
        ...
        resultado = ApiMegaSenaService.sincronizar_banco(
            modo=modo,
            limite=limite,
            teto_concurso=teto,
        )
...
register_premiacao_caixa(index_bp, modality_key="megasena", sorteio_model=SorteioMegaSena, db=db)
