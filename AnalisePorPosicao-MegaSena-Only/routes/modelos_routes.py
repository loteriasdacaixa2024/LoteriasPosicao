...
from services.modelos_service import ModelosMegaSenaService

modelos_bp = Blueprint('modelos', __name__)

@modelos_bp.route('/')
def modelos_index():
    return render_template('modelos.html', modelos=ModelosMegaSenaService.listar_modelos())
...
def api_gerar_modelo(modelo_id: int):
    ...
    try:
        return jsonify(ModelosMegaSenaService.gerar_apostas_modelo(modelo_id))
...
def api_backtesting():
    try:
        resultado = ModelosMegaSenaService.backtesting_modelos()
