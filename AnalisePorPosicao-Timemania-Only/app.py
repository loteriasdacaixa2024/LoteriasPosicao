...
def create_app():
    app = Flask(__name__)
    app.config['SECRET_KEY'] = 'sniper_timemania_secret'
    basedir = os.path.abspath(os.path.dirname(__file__))
    app.config['SQLALCHEMY_DATABASE_URI'] = (
        'sqlite:///' + os.path.join(basedir, 'instance', 'timemania.db')
    )
    app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
    app.config['TEMPLATES_AUTO_RELOAD'] = True
    db.init_app(app)
    extend_config_app(app)
    cc_extend_app(app, 'timemania')
    extend_nav_app(app, 'timemania')
    extend_posicao_app(app, 'timemania')
    extend_concentracao_app(app, 'timemania')
    ...
    extend_ciclo_cobertura_app(app, 'timemania')
    app.register_blueprint(index_bp)
    app.register_blueprint(analise_bp,     url_prefix='/analise')
    register_conferencia_extras(conferencia_bp, 'timemania')
    ...
    with app.app_context():
        import models.sorteio_timemania
    ...
    start_auto_sync_once(
        app,
        "services.api_timemania_service",
        "ApiTimemaniaSService",
    )
...
if __name__ == '__main__':
    app = create_app()
    porta = int(os.getenv('PORT', 5159))
    print("===========================================================")
    print(f"[OK] Servidor Timemania rodando na porta {porta}")
