#!/usr/bin/env python
# -*- coding: utf-8 -*-
r"""Sincroniza histórico completo Timemania (1..último API). Porta 5159."""
...
API_URL = "https://servicebus2.caixa.gov.br/portaldeloterias/api/timemania/"
...
def salvar(concurso, dados):
    dez = extrair_dezenas(dados)
    if not dez:
        return False
    time_num, time_nome = resolver_time(dados)
...
def status(ultimo):
    ...
    mn = db.session.query(db.func.min(SorteioTimemania.concurso)).scalar() or 0
    mx = db.session.query(db.func.max(SorteioTimemania.concurso)).scalar() or 0
    grav = db.session.query(SorteioTimemania.concurso).filter(
        SorteioTimemania.concurso >= 1, SorteioTimemania.concurso <= ultimo
...
def main():
    ...
    with app.app_context():
        ...
        if not ultimo:
            ...
            return 1
        print("=" * 60)
        print("SINCRONIZAÇÃO TIMEMANIA — histórico completo")
        print("Banco: instance/timemania.db | Porta: 5159")
        ...
        while True:
            ...
            for c in faltantes[:limite]:
                print(f"[API] Timemania {c}...", end="", flush=True)
                try:
                    d = buscar(c, timeout)
                    if d and salvar(c, d):
                        db.session.commit()
