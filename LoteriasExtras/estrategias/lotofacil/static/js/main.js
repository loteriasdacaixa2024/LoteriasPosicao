function inferAppRoot() {
    const meta = (document.querySelector('meta[name="app-root"]')?.getAttribute('content') || '').replace(/\/$/, '');
    if (meta) return meta;
...
const APP_ROOT = inferAppRoot();
function apiUrl(path) {
    if (!path.startsWith('/')) path = '/' + path;
    return APP_ROOT + path;
}

const L = window.LOTTERY || {};
const TOTAL = L.total || 25;
const DRAW = L.draw || 15;
const NUM_MIN = (L.min === 0 || L.min) ? L.min : 1;
const SLUG = L.slug || 'lotofacil';
const GRID_COLS = L.grid_cols || 10;
function allNums() {
    return Array.from({ length: TOTAL }, (_, i) => NUM_MIN + i);
}
function dezenasOf(row) {
    if (row && Array.isArray(row.dezenas) && row.dezenas.length) return row.dezenas;
    if (row && Array.isArray(row.bolas) && row.bolas.length) return row.bolas;
    const out = [];
    if (!row) return out;
    for (let i = 1; i <= DRAW; i++) {
        if (row[`dezena${i}`] != null) out.push(row[`dezena${i}`]);
    }
    return out;
}
function modalityColor() {
    return (visualConfig && (visualConfig.modality || visualConfig.lotofacil || {}).cor_modalidade) || 'var(--primary)';
}
...
async function generateIntelligentBase() {
    ...
    try {
        ...
        let freq = {};
        let atraso = {};
        for (const i of allNums()) { freq[i] = 0; atraso[i] = -1; }

        // history is [most_recent, ..., oldest]
        history.forEach((draw, dIdx) => {
            const dezenas = dezenasOf(draw);
            dezenas.forEach(n => {
                freq[n] = (freq[n] || 0) + 1;
        ...
        const groupC = scores.slice(17, TOTAL).map(x => x.dez);
...
function renderIntelligentGame() {
    ...
    const game = intelligentBaseArr.slice(offset, offset + DRAW);
...
function initUniverseGrid() {
    ...
    for (const i of allNums()) {
        ...
        cell.dataset.num = i;
        cell.onclick = () => toggleUniverseNumber(i);
...
async function loadStats() {
    try {
        ...
        if (data.error) {
            ...
            await fetch(apiUrl('/api/debug/populate'));
            loadStats();
            return;
        }
...
function renderStructure(data) {
    ...
    for (const i of allNums()) {
        const idx = i - NUM_MIN;
        const lineIdx = Math.min(Math.floor(idx / GRID_COLS), (latestL || []).length - 1);
        const intensityL = latestL[lineIdx] ? (latestL[lineIdx] / Math.max(1, DRAW / (L.grid_rows || 5))) : 0;
        ...
        const intensityC = latestC[colIdx] ? (latestC[colIdx] / Math.max(1, DRAW / GRID_COLS)) : 0;
...
function renderTemporal(data) {
    ...
    const cycleProgress = ((TOTAL - missingInCycle.length) / TOTAL * 100).toFixed(0);
...
async function generateGames() {
    ...
    if (stratId == 5 && selectedUniverse.length < DRAW) {
        alert(`Selecione pelo menos ${DRAW} números para o universo reduzido.`);
        return;
    }
...
function downloadGames(format) {
    ...
    fetch(apiUrl('/api/download'), {
        ...
        .then(blob => {
            ...
            a.href = url;
            a.download = `${SLUG}_jogos_${count}.${format}`;
...
function renderMultiSimulator() {
    ...
    simulatorGrids.forEach((numbers, idx) => {
        ...
        info.style.cssText = `min-width: 7rem; flex: 0 1 auto; display: flex; align-items: center; gap: 8px;`;
        info.innerHTML = `
            ...
            <span style="font-weight: 800; font-size: 0.75rem; color: #555;">APOSTA ${idx + 1} <span style="color: var(--primary); font-size: 0.65rem;">(${numbers.size}/${DRAW})</span></span>
        `;
        row.appendChild(info);

        const cellsContainer = document.createElement('div');
        cellsContainer.style.cssText = `display: flex; flex-wrap: wrap; gap: 3px; flex: 1 1 12rem; min-width: 0;`;
        ...
        for (const i of allNums()) {
            ...
            cell.style.cssText = `
                width: clamp(1.5rem, 7vw, 1.75rem); height: clamp(1.5rem, 7vw, 1.75rem); border-radius: 50%; border: 1px solid #ddd;
                display: flex; align-items: center; justify-content: center;
                font-size: 0.7rem; font-weight: 700; cursor: pointer;
                background: ${isActive ? modalityColor() : '#f9f9f9'};
        ...
        if (isHibrido && numbers.size > 0) {
            ...
            let repeated = 0;
            if (historicalResults.length > 0) {
                const prevNums = dezenasOf(historicalResults[0]);
...
function downloadManualBets() {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `${SLUG}_apostas_manuais_${count}.txt`;
    document.body.appendChild(a);
    a.click();
...
function renderVisualGrids() {
    ...
    historicalResults.forEach((res, idx) => {
        const dezenas = dezenasOf(res);

        const prevDezenas = historicalResults[idx + 1] ? dezenasOf(historicalResults[idx + 1]) : [];
        ...
        const info = document.createElement('div');
        info.style.cssText = `min-width: 5.5rem; flex: 0 1 auto; font-size: 0.7rem; font-weight: 800; color: #888;`;
        info.innerHTML = `
        ...
        for (const i of allNums()) {
            ...
            cell.style.cssText = `
                width: clamp(1.5rem, 7vw, 1.75rem); height: clamp(1.5rem, 7vw, 1.75rem); border-radius: 50%; border: ${border};
                display: flex; align-items: center; justify-content: center;
