# -*- coding: utf-8 -*-
"""Gerador por Posição — plugins Repetição, Sniper e Comportamento (multi-modalidade)."""
...
def _posicional_ultimo_par(analise: Dict[str, Any], num_pos: int) -> Dict[int, int]:
    out: Dict[int, int] = {}
...
def carregar_plugins_ctx(
    ...
    if opcoes.usar_repeticao or opcoes.usar_sniper:
        ...
        rep = RepeticaoConcursosService(modality_key).analisar_completo(modo="posicional")
        ctx["repeticao"] = rep
        ctx["rep_por_pos"] = _posicional_ultimo_par(rep, spec.num_posicoes)
    ...
    if opcoes.usar_sniper:
        ...
        intel = get_inteligente_service(modality_key)
        if intel:
    ...
    if opcoes.usar_comportamento:
        ...
        if tem_gerador_comportamento(modality_key):
            comp_svc = get_comportamento_service(modality_key)
            comp = comp_svc.analisar(janela=10, base_estatistica="geral")
            ctx["comportamento"] = comp
            alvos = (comp.get("criterios_sugeridos") or {}).get("alvos") or {}
            ctx["comp_alvos"] = alvos
            ctx["comportamento_svc"] = comp_svc
...
def criar_score_boost(
    opcoes: OpcoesGeradorPosicao,
    ctx: Dict[str, Any],
) -> Optional[Callable[[int, Dict[str, Any], float], float]]:
    opcoes = opcoes.normalizar()
...
def criar_validador_comportamento(
    ...
    if not opcoes.usar_comportamento or not ctx.get("comportamento"):
        return None
    comp_svc = ctx.get("comportamento_svc")
    if not comp_svc:
        return None
    ...
    def validar(dezenas: List[int], extras: Optional[Dict[str, Any]] = None) -> bool:
        try:
            val = comp_svc.validar_selecao_panorama_api(
                dezenas=dezenas,
...
def gerar_apostas_com_plugins(
    ...
    opcoes = opcoes.normalizar()
    ctx = plugins_ctx if plugins_ctx is not None else carregar_plugins_ctx(opcoes, modality_key, spec)
    score_boost = criar_score_boost(opcoes, ctx)
    validar = criar_validador_comportamento(opcoes, ctx, spec)
    ...
    while len(apostas) < qtd and tentativas < max_total:
        ...
        if chave in vistos:
            continue
        vistos.add(chave)
        item = formatar_aposta_posicional(ordem, spec)
        if validar and ctx.get("comportamento_svc"):
            try:
                val = ctx["comportamento_svc"].validar_selecao_panorama_api(
                    dezenas=ordem,
                    base_estatistica="geral",
                    rank_escolhido=1,
                    modo=opcoes.modo_comportamento,
                    dezenas_por_jogo=spec.num_posicoes,
                )
