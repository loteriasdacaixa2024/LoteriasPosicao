...
def gerar_apostas_dna(
    ...
    if spec.motor == "colunas":
        return {
            "sucesso": False,
            "erro": "O gerador por DNA usa conjunto de dezenas; Super Sete é por colunas.",
        }
    ...
    try:
        from resumo_modalidade.service import _carregar_sorteios
        historico = {frozenset(int(x) for x in s["dezenas"]) for s in _carregar_sorteios(spec)}
    except Exception:
        historico = set()
