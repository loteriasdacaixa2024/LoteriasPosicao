    ...
    *,
    ...
    if detalhe:
        ...
        paragrafos.append(
            ...
            f"{zeros} tiveram zero dezenas da categoria Ausentes no resultado oficial."
        )
        mix = []
        for d in detalhe:
            mix.append(
                f"#{d['concurso']}: {d['nucleo_n']} núcleo + {d['baixa_n']} baixa + "
                f"{d['ausentes_n']} ausente(s)"
            )
        paragrafos.append("Composição observada: " + "; ".join(mix) + ".")
    ...
    if slice50.get("n"):
        paragrafos.append(
            f"No walk-forward dos últimos {slice50['n']} concursos (janela {janela}, "
            f"sem vazar o próprio resultado na classificação): média de "
            f"{comp.get('nucleo', 0)} dezena(s) do núcleo recorrente, "
            f"{comp.get('baixa_presenca', 0)} de baixa presença e "
            f"{comp.get('ausentes', 0)} ausente(s) por sorteio."
            + (
    ...
    paragrafos.append(
        "O teto bruto (quantas dezenas do resultado cabem no pool) favorece o conjunto maior. "
        "Por isso Ausentes + Baixa cobrem mais dezenas — o pool tem ~26 números. "
        "A densidade (acertos / tamanho do pool) é o critério justo para montar um jogo de 7."
    )
    if rec in ("nucleo_baixa", "nucleo_baixa_1aus"):
        paragrafos.append(
            "Não deixe Ausentes dominarem a aposta: 0 ocorrências na janela não tornam a dezena "
            "mais provável, e o núcleo que a análise marcou como forte/repetido precisa entrar no jogo."
        )
        paragrafos.append(
            "Recomendação: Pool principal = Núcleo + Baixa presença; "
            "Ausentes ficam num pool secundário de recuperação (no máximo 1 dezena por jogo)."
        )
    else:
        paragrafos.append(
            f"Neste recorte a densidade favoreceu «{rec_label}». "
            "Mesmo assim vale não deixar Ausentes dominarem a composição de cada jogo."
        )
    ...
    return {
        ...
        "problema_atual": (
            "A estratégia atual monta Pool = Ausentes + Baixa presença e, "
            "com isso, descarta o núcleo que a própria análise classificou como forte/repetido."
        ),
    }
    ...
    *,
    ...
    if not wf:
        return {
            "ok": False,
            "erro": (
                f"Histórico insuficiente para pós-análise walk-forward "
                f"(precisa de mais de {janela} concursos com repetidas)."
            ),
        }
    ...
    return {
        ...
        "conclusao": conclusao,
        "nota": (
            "Os cartões cruzam o resultado oficial (7 dezenas) com os grupos desta janela. "
            "O backtest é walk-forward: cada concurso passado usa só os N anteriores. "
            "Teto = quantas dezenas do resultado estavam no pool da estratégia."
        ),
    }
