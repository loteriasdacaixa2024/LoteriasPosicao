...
def build_geradores_elite_blueprint(modality_key: str) -> Blueprint:
    ...
    def index():
        ...
        return render_template(
            ...
            cfg=cfg,
            tem_ciclo_cobertura=tem_ciclo_cobertura(modality_key),
        )
    ...
    def ciclo_apostas_page():
        ...
        return render_template(
            ...
            meses_cores=meses_cores,
            meses=meses,
            has_mes=(modality_key == "diadesorte"),
        )
    ...
    @bp.route("/api/ciclo-apostas/recorrencia/validar", methods=["POST"])
    def api_ciclo_recorrencia_validar():
        from ciclo_cobertura.gerador_padroes_recorrencia import validar_request
        from ciclo_cobertura.specs import tem_ciclo_cobertura

        if not tem_ciclo_cobertura(modality_key):
            return jsonify({"ok": False, "erro": "Indisponível para esta modalidade."}), 404
        data = request.get_json(silent=True) or {}
        out = validar_request(modality_key, data)
        return jsonify(out), (200 if out.get("ok") else 400)

    @bp.route("/api/ciclo-apostas/recorrencia/completar", methods=["POST"])
    def api_ciclo_recorrencia_completar():
        from ciclo_cobertura.gerador_padroes_recorrencia import completar_request
        from ciclo_cobertura.specs import tem_ciclo_cobertura

        if not tem_ciclo_cobertura(modality_key):
            return jsonify({"ok": False, "erro": "Indisponível para esta modalidade."}), 404
        data = request.get_json(silent=True) or {}
        out = completar_request(modality_key, data)
        return jsonify(out), (200 if out.get("ok") else 400)

    @bp.route("/api/ciclo-apostas/recorrencia/gerar", methods=["POST"])
    def api_ciclo_recorrencia_gerar():
        from ciclo_cobertura.gerador_padroes_recorrencia import gerar_request
        from ciclo_cobertura.specs import tem_ciclo_cobertura

        if not tem_ciclo_cobertura(modality_key):
            return jsonify({"ok": False, "erro": "Indisponível para esta modalidade."}), 404
        data = request.get_json(silent=True) or {}
        out = gerar_request(modality_key, data)
        code = 200 if out.get("ok") else 400
        return jsonify(out), code
    ...
    @bp.route("/api/construtor-construcoes/ciclo-visao")
    def api_construtor_ciclo_visao():
        if not _construtor_ok():
            return jsonify({"sucesso": False, "erro": "Indisponível."}), 404
        try:
            return jsonify(_construtor_svc().visao_ciclo())
        except Exception as e:
            return jsonify({"sucesso": False, "erro": str(e)}), 500
    ...
    @bp.route("/api/construtor-construcoes/construcao/<int:construcao_id>/refinar", methods=["POST"])
    def api_construtor_refinar(construcao_id):
        if not _construtor_ok():
            return jsonify({"sucesso": False, "erro": "Indisponível."}), 404
        data = request.get_json(silent=True) or {}
        try:
            return jsonify(_construtor_svc().refinar_construcao(
                construcao_id,
                modo=data.get("modo") or "inteligente",
                intensidade=data.get("intensidade") or "leve",
                qtd_apostas=data.get("qtd_apostas") or "todas",
    ...
    def api_construtor_export_txt(construcao_id):
        ...
        try:
            ...
            return jsonify(_construtor_svc().exportar_txt(
                ...
                somente_refinadas=bool(data.get("somente_refinadas")),
            ))
        except Exception as e:
            return jsonify({"sucesso": False, "erro": str(e)}), 500
