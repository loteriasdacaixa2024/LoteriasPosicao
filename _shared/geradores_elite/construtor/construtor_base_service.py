...
class ConstrutorBaseService:
    ...
    def ui_config(cls) -> Dict[str, Any]:
        ...
        try:
            from configuracoes.regras_modalidade import get_regras
            regras = get_regras(sp.modality_key)
            out["precos_map"] = {str(k): v for k, v in (regras.get("precos_map") or {}).items()}
            out["tabela_precos"] = regras.get("tabela_precos") or []
        except Exception:
            out["precos_map"] = {}
            out["tabela_precos"] = []
        if sp.has_mes:
    ...
    @classmethod
    def visao_ciclo(cls) -> Dict[str, Any]:
        """Snapshot do ciclo atual para o Construtor — não altera o conjunto-base."""
        try:
            ciclo = cls._ciclo_service().obter_ciclo_atual() or {}
        except Exception as e:
            return {"sucesso": False, "erro": str(e)}
        if not ciclo:
            return {"sucesso": False, "erro": "Sem ciclo em andamento."}
        sp = cls._spec()
        univ = int(sp.universo or 0)
        dmin = int(sp.dezena_min or 1)
        falt = sorted({
            int(x) for x in (
                ciclo.get("dezenas_faltantes") or ciclo.get("dezenas_pendentes") or []
            ) if x is not None and str(x).strip() != ""
        })
        saidas = sorted({
            int(x) for x in (
                ciclo.get("dezenas_sorteadas") or ciclo.get("dezenas_saidas") or []
            ) if x is not None and str(x).strip() != ""
        })
        if not saidas and univ:
            todas = set(range(dmin, dmin + univ))
            saidas = sorted(todas - set(falt))
        n_falt = len(falt)
        n_saidas = len(saidas) or int(
            ciclo.get("total_sorteadas") or ciclo.get("total_dezenas_saidas") or 0
        )
        pct = ciclo.get("percentual_completo")
        if pct is None and univ:
            pct = round(100.0 * n_saidas / univ, 1)
        return {
            "sucesso": True,
            "ciclo_num": ciclo.get("ciclo_num") or ciclo.get("numero_ciclo"),
            "percentual": pct,
            "faltantes": falt,
            "sorteadas": saidas,
            "total_faltantes": n_falt,
            "total_sorteadas": n_saidas,
            "universo": univ,
            "concursos_no_ciclo": ciclo.get("concursos_no_ciclo") or ciclo.get("quantidade_concursos"),
            "em_andamento": bool(ciclo.get("em_andamento", True)),
        }
        ...
        *,
        ...
        for round_i in range(MAX_ROUNDS):
            ...
            for ap in resultado.get("apostas") or []:
                ...
                if len(dz) != k:
                    continue
                if not aposta_pertence_ao_pool(dz, pool):
                    continue
            ...
            for ap in novos:
                ...
                if chave in excluidas:
                    continue
                if len(dz) != k or not aposta_pertence_ao_pool(dz, pool):
                    continue
        ...
        if len(apostas_ok) < QTD_APOSTAS_FIXA:
            ultimo = ultimo_resultado or {}
            problemas = list(ultimo.get("padroes_problema") or [])
            if padroes_sel:
                return {
                    "sucesso": False,
                    "erro_codigo": "duplicidade_total" if not problemas else "pool_insuficiente",
                    "erro": (
                        "⚠️ Construção não gerada. Não foi possível completar "
                        f"{QTD_APOSTAS_FIXA} apostas inéditas com os Padrões II "
                        f"selecionados usando apenas as dezenas do POOL "
                        f"(obtidas {len(apostas_ok)}). "
                        "Sequências totalmente iguais às já existentes foram rejeitadas. "
                        "Nenhuma dezena fora do POOL foi utilizada."
                    ),
                    "qtd_obtidas": len(apostas_ok),
                    "rejeitadas_validacao": rejeitadas_vg,
                    "padroes_problema": problemas,
                    "oferece_sugestao": bool(problemas),
                    "padroes_ii_ativos": True,
                    "padroes_ii_usados": ultimo.get("padroes_ii_usados") or padroes_sel,
                }
            return {
                "sucesso": False,
                "erro_codigo": "construcao_nao_gerada",
                "erro": (
        ...
        preco = cls._preco_lote(k, len(apostas_ok))
        preco_sessao = cls._preco_lote(k, len(apostas_ok) * qtd_sessao)
        return {
            "sucesso": True,
            ...
            "similaridade_max_anterior": resultado.get("similaridade_max_anterior"),
            "diferenca_min_pct": resultado.get("diferenca_min_pct"),
            "limiar_similaridade": sim_max,
            "limiar_similaridade_pct": round(sim_max * 100.0, 1),
            "similaridade_min_pct": sim_min,
            "padroes_ii_ativos": bool(resultado.get("padroes_ii_ativos")),
            "padroes_ii_usados": resultado.get("padroes_ii_usados") or [],
            "padroes_problema": resultado.get("padroes_problema") or [],
            "oferece_sugestao": bool(resultado.get("oferece_sugestao")),
            "valor_unitario": preco.get("unitario"),
            "valor_unitario_fmt": preco.get("unitario_fmt") or "",
            "valor_total": preco.get("total"),
            "valor_total_fmt": preco.get("total_fmt") or "",
            "valor_sessao": preco_sessao.get("total"),
            "valor_sessao_fmt": preco_sessao.get("total_fmt") or "",
        }
    ...
    @classmethod
    def _faltantes_ciclo(cls) -> List[int]:
        try:
            ciclo = cls._ciclo_service().obter_ciclo_atual() or {}
        except Exception:
            return []
        raw = ciclo.get("dezenas_faltantes") or ciclo.get("dezenas_pendentes") or []
        return sorted({int(x) for x in raw if x is not None and str(x).strip() != ""})

    @classmethod
    def refinar_construcao(
        cls,
        construcao_id: int,
        *,
        ...
        if not sessao:
            return {"sucesso": False, "erro": "Sessão não encontrada."}
        originais = [a.dezenas_lista() for a in construcao.apostas]
        if not originais:
            return {"sucesso": False, "erro": "A construção não tem apostas."}
        pool = sessao.conjunto_lista()
        sorteios = [row["dezenas"] for row in cls.listar_concursos(80) if row.get("dezenas")]
        resultado = refinar_apostas(
            ...
            variacoes=variacoes,
            distancia=distancia,
            faltantes_ciclo=cls._faltantes_ciclo(),
            sorteios=sorteios,
        )
        if not resultado.get("ok"):
            return {"sucesso": False, "erro": resultado.get("erro") or "Falha no refinamento."}
        extra = construcao.extra_dict()
        extra["refinamento"] = resultado
        construcao.extra_json = json.dumps(extra, ensure_ascii=False)
        db.session.commit()
        return {
            "sucesso": True,
            "refinamento": resultado,
            "sessao": cls._serializar_sessao(sessao, True),
        }
    ...
    def exportar_txt(
        ...
        *,
        meses_pre: Optional[List[int]] = None,
        somente_refinadas: bool = False,
    ) -> Dict[str, Any]:
        sp = cls._spec()
        construcao = db.session.get(ConstrutorConstrucao, construcao_id)
        if not construcao:
            return {"sucesso": False, "erro": "Construção não encontrada."}
        if somente_refinadas:
            pares = (construcao.extra_dict().get("refinamento") or {}).get("apostas") or []
            apostas = [{"dezenas": p.get("refinada") or []} for p in pares if p.get("refinada")]
            if not apostas:
                return {"sucesso": False, "erro": "Não há apostas refinadas nesta construção. Refine antes de exportar."}
        else:
        ...
        if somente_refinadas:
            nome_arq = f"construcao_{construcao.numero}_refinadas.txt"
    ...
    def _serializar_construcao(cls, c: ConstrutorConstrucao) -> Dict[str, Any]:
        ...
        n_apostas = len(out["apostas"])
        k = 0
        if out["apostas"]:
            k = len(out["apostas"][0].get("dezenas") or [])
        preco = cls._preco_lote(k, n_apostas)
        out["qtd_apostas"] = n_apostas
        out["valor_unitario"] = preco.get("unitario")
        out["valor_unitario_fmt"] = preco.get("unitario_fmt") or ""
        out["valor_total"] = preco.get("total")
        out["valor_total_fmt"] = preco.get("total_fmt") or ""
        ...
        ref = c.extra_dict().get("refinamento")
    ...
    def _serializar_sessao(cls, s: ConstrutorSessao, incluir_construcoes: bool) -> Dict[str, Any]:
        ...
        n_apostas = sum(len(c.apostas) for c in s.construcoes)
        preco = cls._preco_lote(s.dezenas_por_aposta, n_apostas)
        out["qtd_apostas"] = n_apostas
        out["valor_unitario"] = preco.get("unitario")
        out["valor_unitario_fmt"] = preco.get("unitario_fmt") or ""
        out["valor_total"] = preco.get("total")
    ...
    @classmethod
    def _preco_lote(cls, n_dezenas: int, n_apostas: int) -> Dict[str, Any]:
        try:
            from configuracoes.regras_modalidade import preco_lote
            return preco_lote(cls._spec().modality_key, n_dezenas, n_apostas)
        except Exception:
            return {
                "unitario": None,
                "total": None,
                "unitario_fmt": "",
                "total_fmt": "",
                "dezenas_por_aposta": n_dezenas,
                "qtd_apostas": n_apostas,
            }
