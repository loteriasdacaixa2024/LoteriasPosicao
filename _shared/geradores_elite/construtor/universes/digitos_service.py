...
class ConstrutorDigitosService:
    ...
    def guia(cls, modality_key: str) -> Dict[str, Any]:
        ...
        linhas = dig.get("linhas") or []
        ultimo_dz: List[int] = []
        if linhas:
            # analisar_digitos: linhas[0] = concurso mais recente
            ultimo_dz = [int(x) for x in (linhas[0].get("dezenas") or [])]
        pad = max(1, int(sp.dezena_fmt_width or 2))
        return {
            "sucesso": True,
            ...
            "ultimo_dezenas": ultimo_dz,
            "ultimo_dezenas_fmt": " ".join(f"{n:0{pad}d}" for n in ultimo_dz),
        }
    ...
    def gerar_inteligente(
        ...
        *,
        ...
        # Histórico/memória ficam a cargo do pipeline global (política «marcar»).
        # Aqui NÃO descartamos: pool fechado (ex. 0,1,2 → só 8 combos) precisa
        # poder regenerar o mesmo lote sem falso «não dá para gerar».
        try:
            from geradores_elite.validacao.validador_global import ValidadorGeradoresElite
            antes = list(result.get("apostas") or [])
            result = ValidadorGeradoresElite.aplicar(
                result,
                origem="construtor_digitos",
                modality_key=modality_key,
                campo="apostas",
                checar_historico=True,
                checar_memoria=False,
                registrar_aprovadas=False,
            )
            result["qtd_geradas"] = len(result.get("apostas") or [])
            # Se o histórico oficial removeu 100%, devolve o lote com aviso
            # (o pipeline ainda marca quais já saíram).
            if result["qtd_geradas"] == 0 and antes:
                for i, item in enumerate(antes, 1):
                    if isinstance(item, dict):
                        item["linha"] = i
                result["apostas"] = antes
                result["qtd_geradas"] = len(antes)
                result["sucesso"] = True
                result["aviso"] = (
                    (result.get("aviso") + " " if result.get("aviso") else "")
                    + "Todas as combinações deste pool já saíram no histórico oficial "
                    + "— lote mantido para visualização/comparação."
                )
        except Exception:
            pass
