...
def aposta_pertence_ao_pool(aposta: Sequence[int], pool: Sequence[int]) -> bool:
    permitido = set(int(x) for x in pool)
    return all(int(d) in permitido for d in aposta)


def universo_dos_limites(limites: Optional[Dict[str, Tuple[int, int]]] = None) -> List[int]:
    if not limites:
        return list(range(1, 32))
    lo = min(int(v[0]) for v in limites.values())
    hi = max(int(v[1]) for v in limites.values())
    return list(range(lo, hi + 1))


def sugerir_ajuste_pool(
    pool: Sequence[int],
    padrao: Sequence[int],
    universo: Optional[Sequence[int]] = None,
) -> Dict[str, Any]:
    """Sugestão de dezenas (fora do pool) para viabilizar o padrão. Nunca altera o pool."""
    pool_set = set(int(x) for x in pool)
    need = Counter(int(x) for x in padrao)
    groups = pool_por_digito_inicial(pool)
    univ = [int(x) for x in (universo or [])]
    faltas: List[Dict[str, Any]] = []
    candidatas: List[int] = []
    for dig, qtd in sorted(need.items()):
        no_pool = len(groups.get(dig, []))
        falta = qtd - no_pool
        if falta <= 0:
            continue
        cands = [d for d in univ if (d // 10) == dig and d not in pool_set]
        faltas.append({
            "digito": int(dig),
            "precisa": int(qtd),
            "no_pool": int(no_pool),
            "faltam": int(falta),
            "dezenas_candidatas": cands,
        })
        candidatas.extend(cands[:falta])
    return {
        "nao_altera_pool": True,
        "faltas": faltas,
        "dezenas_candidatas": sorted(set(candidatas)),
    }


def diagnosticar_padroes_ii(
    pool: Sequence[int],
    padroes: Sequence[str],
    k: int,
    universo: Optional[Sequence[int]] = None,
) -> Dict[str, Any]:
    """Classifica cada Padrão II selecionado como viável ou insuficiente no POOL."""
    viaveis: List[str] = []
    problemas: List[Dict[str, Any]] = []
    for raw in padroes:
        pstr = " ".join(str(x) for x in parse_padrao(raw))
        digs = parse_padrao(pstr)
        if len(digs) != k:
            problemas.append({
                "padrao": pstr or str(raw),
                "motivo": "tamanho_incompativel",
                "detalhe": (
                    f"O padrão tem {len(digs)} dígito(s); a construção exige {k} dezenas."
                ),
                "sugestao": None,
            })
            continue
        if padrao_viavel(digs, pool):
            viaveis.append(pstr)
            continue
        sug = sugerir_ajuste_pool(pool, digs, universo)
        problemas.append({
            "padrao": pstr,
            "motivo": "pool_insuficiente",
            "detalhe": (
                "As dezenas selecionadas no POOL não são suficientes para gerar "
                "uma aposta compatível com este padrão."
            ),
            "sugestao": sug,
        })
    return {"viaveis": viaveis, "problemas": problemas}
...
def _aposta_ok_diversidade(
    ...
    permitir_igualdade_parcial: bool = False,
) -> bool:
    chave = frozenset(ap)
    if chave in usadas:
        return False
...
def gerar_construcao(
    ...
    *,
    ...
    - padroes_selecionados: Padrões II da análise — só entram se o usuário selecionou.
      Desligados/não selecionados são ignorados (não há aplicação automática).
    """
    qtd_alvo = max(1, int(quantidade or QTD_APOSTAS_FIXA))
    pool = sorted(set(int(x) for x in pool))
    rng = random.Random(seed)
    construcoes_anteriores = construcoes_anteriores or []
    universo = universo_dos_limites(limites)
    selecionados_raw = [
        " ".join(str(x) for x in parse_padrao(p))
        for p in (padroes_selecionados or [])
        if parse_padrao(p)
    ]
    padroes_ii_ativos = bool(selecionados_raw)

    if len(pool) < k:
        return {
            "sucesso": False,
            "erro_codigo": "pool_insuficiente",
            "erro": (
                "⚠️ POOL insuficiente. O conjunto-base precisa de pelo menos "
                f"{k} dezenas para formar uma aposta."
            ),
            "oferece_sugestao": False,
        }

    diag_ii = diagnosticar_padroes_ii(pool, selecionados_raw, k, universo) if padroes_ii_ativos else None
    selecionados = list((diag_ii or {}).get("viaveis") or [])
    problemas_ii = list((diag_ii or {}).get("problemas") or [])

    if padroes_ii_ativos and not selecionados:
        nomes = ", ".join(p.get("padrao") or "?" for p in problemas_ii) or "selecionado"
        motivos = {p.get("motivo") for p in problemas_ii}
        codigo = (
            "padrao_nao_atendido"
            if motivos and motivos <= {"tamanho_incompativel"}
            else "pool_insuficiente"
        )
        if codigo == "padrao_nao_atendido":
            erro_txt = (
                "⚠️ Padrão não atendido. O Padrão II selecionado exige uma "
                f"configuração ({nomes}) que não pode ser formada apenas com as "
                "dezenas atualmente disponíveis no POOL, ou o tamanho do padrão "
                "não coincide com a quantidade de dezenas da construção."
            )
        else:
            erro_txt = (
                "⚠️ POOL insuficiente. Não foi possível gerar apostas para o(s) "
                f"Padrão(ões) II selecionado(s) ({nomes}). "
                "As dezenas atualmente selecionadas no POOL não são suficientes "
                "para atender às regras desse padrão. Nenhuma dezena fora do POOL foi usada."
            )
        return {
            "sucesso": False,
            "erro_codigo": codigo,
            "erro": erro_txt,
            "padroes_problema": problemas_ii,
            "oferece_sugestao": True,
        }

    if padroes_ii_ativos:
        # Com Padrões II ativos a montagem não usa B/M/A; dist é só metadado.
        dist = calcular_distribuicao(
            estrategia, pool, k, personalizada, limites=limites
        )
        if sum(dist.values()) != k:
            dist = {"baixas": k, "medias": 0, "altas": 0}
            if limites and "isolada" in limites:
                dist["isolada"] = 0
    else:
        ...
        if not ok:
            return {"sucesso": False, "erro": msg, "erro_codigo": "estrategia"}
    ...
    for tentativa in range(max_tentativas):
        ...
        if usar_padroes:
            if padroes_ii_ativos:
                for i in range(qtd_alvo):
                    ...
                    alvos_padrao.append(parse_padrao(pstr))
            else:
        ...
        for i in range(qtd_alvo):
            ...
            for _try in range(220):
                ...
                elif alvo is not None and _try < 120:
                    ap = _montar_por_padrao(pool, alvo, rng)
                else:
                    dist_uso = dist
                ...
                if len(ap) != k:
                    continue
                if not aposta_pertence_ao_pool(ap, pool):
                    continue
                pstr = padrao_inicial_de(ap)
                if padroes_ii_ativos and alvo is not None:
                    if pstr != " ".join(str(x) for x in alvo):
                        continue
                if (not padroes_ii_ativos) and pstr in padroes_usados and _try < 160 and len(padroes_usados) < i + 1:
                    continue
                if not _aposta_ok_diversidade(
                    ap, usadas, padroes_usados, historico_sorteados,
                    apostas_excluidas, jaccard_max_lote, apostas,
                    exigir_padrao_distinto=not padroes_ii_ativos,
                    permitir_igualdade_parcial=padroes_ii_ativos,
                ):
                    continue
        ...
        if falhou or len(apostas) < qtd_alvo:
            continue
        if any(len(a) != k or not aposta_pertence_ao_pool(a, pool) for a in apostas):
            continue
        ...
        meta = {
            ...
            "padroes_ii_ativos": padroes_ii_ativos,
            "padroes_ii_usados": list(selecionados) if padroes_ii_ativos else [],
            "padroes_problema": problemas_ii if padroes_ii_ativos else [],
        }
        if padroes_ii_ativos and problemas_ii:
            nomes = ", ".join(p.get("padrao") or "?" for p in problemas_ii)
            extra_aviso = (
                f"Padrão II — {nomes}: as dezenas do POOL não são suficientes "
                "para este padrão (não foi usado). Nenhuma dezena foi adicionada ao POOL."
            )
            meta["aviso"] = extra_aviso
            meta["oferece_sugestao"] = True
        ...
        if score > melhor_score or (score == melhor_score and diff_min > melhor_diff):
            ...
            melhor["aviso"] = (
                f"{meta.get('aviso')} {aviso_sim}".strip() if meta.get("aviso") else aviso_sim
            )
    ...
    if padroes_ii_ativos:
        return {
            "sucesso": False,
            "erro_codigo": "duplicidade_total",
            "erro": (
                "⚠️ Construção não gerada. Todas as combinações possíveis para o "
                "padrão selecionado foram descartadas porque resultariam em "
                "sequências totalmente iguais às já existentes, ou o POOL não "
                "permite montar apostas inéditas sem sair das dezenas marcadas."
            ),
            "padroes_problema": problemas_ii,
            "oferece_sugestao": bool(problemas_ii),
        }
    return {
        "sucesso": False,
