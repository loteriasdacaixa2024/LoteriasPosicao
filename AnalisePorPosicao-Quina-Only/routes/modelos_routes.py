...
from services.modelos_service import ModelosQuinaService

modelos_bp = Blueprint('modelos', __name__)

@modelos_bp.route('/')
def modelos_index():
    modelos = ModelosQuinaService.listar_modelos()
...
def api_gerar_modelo(modelo_id: int):
    ...
    try:
        return jsonify(ModelosQuinaService.gerar_apostas_modelo(modelo_id))
...
def api_backtesting():
    try:
        resultado = ModelosQuinaService.backtesting_modelos()
