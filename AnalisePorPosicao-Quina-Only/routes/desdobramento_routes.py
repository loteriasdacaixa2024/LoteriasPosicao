...
def desdobramento_index():
    try:
        ciclo_info = CicloQuinaService.obter_ciclo_atual()
...
def api_ciclo():
    try:
        return jsonify({"status": "success", **CicloQuinaService.obter_ciclo_atual()})
    except Exception as e:
...
def api_sugestoes_colunas():
    try:
        return jsonify({"status": "success", "sugestoes": DesdobramentoQuinaService.obter_sugestoes_colunas()})
    except Exception:
...
def api_desdobrar():
    try:
        ...
        id_salvo = DesdobramentoQuinaService.salvar_desdobramento(nome, numeros, modo)
        detalhes = DesdobramentoQuinaService.buscar_por_id(id_salvo)
        if not detalhes:
...
def api_listar_desdobramentos():
    try:
        ...
        return jsonify({"status": "success", "desdobramentos": DesdobramentoQuinaService.listar_todos(tipo)})
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500
...
def api_buscar_desdobramento(id):
    try:
        detalhes = DesdobramentoQuinaService.buscar_por_id(id)
        if not detalhes:
...
def api_deletar_desdobramento(id):
    try:
        if not DesdobramentoQuinaService.deletar_por_id(id):
            return jsonify({"status": "error", "message": "Desdobramento não encontrado."}), 404
